  ...
  <div id="app">
    ...
    <fieldset class="search-container" style="text-align: center;margin: 50px 0;">
      <form class="layui-form" style="margin: 10px 0;">
        ...
        <div class="layui-inline" style="margin-left: 30px;">
          ...
          <button v-if="isAuth('siliujibaoming','新增')" @click="jump('../siliujibaoming/add.jsp')" type="button" class="layui-btn btn-theme">
    ...
    <div class="product">
      <div class="proBox" v-for="(item,index) in dataList" v-bind:key="index">
        ...
        <div class="proBoxtxt">
          <p>{{item.xiangmumingcheng}}&nbsp;&nbsp;<span style="color: red;" v-if="item.price">{{item.price}} RMB</span></p>
          <a :href="'../siliujibaoming/detail.jsp?id='+item.id">查看详情</a>
        </div>
      </div>
      <div class="clear"></div>
  ...
  <script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
        ...
        if (jquery('#kaoshibianhao').val()) {
          param['kaoshibianhao'] = jquery('#kaoshibianhao').val() ? '%' + jquery('#kaoshibianhao').val() + '%' : '';
        }  
        if (jquery('#leibie').val()) {
          param['leibie'] = jquery('#leibie').val() ? '%' + jquery('#leibie').val() + '%' : '';
        }  
        // 获取列表数据
        http.request('siliujibaoming/list', 'get', param, function(res) {
